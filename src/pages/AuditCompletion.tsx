...
import { Alert, Box, Button, Card, CardContent, Chip, Container, Dialog, DialogActions, DialogContent, DialogTitle, Divider, Grid, IconButton, MenuItem, Stack, Table, TableBody, TableCell, TableContainer, TableHead, TablePagination, TableRow, TextField, Tooltip, Typography } from '@mui/material';
...
const auditTypeOptions = ['all', '2W', '3W', '4W', 'JBR'];
const emptyColumnFilters = (): Record<CompletionKey, string> =>
  Object.fromEntries(keys.map((key) => [key, ''])) as Record<CompletionKey, string>;
...
const AuditCompletion = () => {
  ...
  const [columnFilters, setColumnFilters] = useState<Record<CompletionKey, string>>(emptyColumnFilters());
  ...
  const setColumnFilter = (key: CompletionKey, value: string) => {
    setColumnFilters((prev) => ({ ...prev, [key]: value }));
    setPage(0);
  };

  const filteredRecords = useMemo(() => auditTypeFilter === 'all' ? records : records.filter((record) => record.auditType.trim().toUpperCase() === auditTypeFilter), [records, auditTypeFilter]);
  const tableRecords = useMemo(() => {
    return filteredRecords.filter((record) =>
      keys.every((key) => {
        const filterValue = columnFilters[key].trim().toLowerCase();
        return !filterValue || record[key].toLowerCase().includes(filterValue);
      })
    );
  ...
  const paginatedRecords = tableRecords.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage);
  ...
  const handleDownload = () => {
    ...
    const detail = XLSX.utils.aoa_to_sheet([headers.slice(0, -1), ...filteredRecords.map((record) => keys.map((key) => record[key]))]);
    detail['!cols'] = widths.map((width) => ({ wch: Math.ceil(width / 8) }));
    const summarySheet = XLSX.utils.aoa_to_sheet([['S.No.', 'State', 'No. of Audits', 'Total No. of Unique Items'], ...summary.map((item, index) => [index + 1, item.state, item.auditCount, item.uniqueItems]), ['', 'Total', filteredRecords.length, totalUniqueItems]]);
  ...
  const hasActiveFilters = keys.some((key) => columnFilters[key].trim());

  return <Container maxWidth="xl" sx={{ py: { xs: 3, md: 4 }, pb: 8 }}>
    <Stack direction={{ xs: 'column', lg: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', lg: 'center' }} spacing={2.5} sx={{ mb: 3 }}><Box><Typography variant="h4" fontWeight={850} color="#123B45">Audit Completion</Typography><Typography color="text.secondary" sx={{ mt: 0.5 }}>Manage completed audits month by month.</Typography></Box><Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.25} width={{ xs: '100%', lg: 'auto' }}><TextField label="Completion month" type="month" value={month} onChange={(event) => { setMonth(event.target.value || monthValue()); setPage(0); }} InputLabelProps={{ shrink: true }} sx={{ minWidth: 180 }} /><TextField select label="Audit type" value={auditTypeFilter} onChange={(event) => { setAuditTypeFilter(event.target.value); setPage(0); }} sx={{ minWidth: 140 }}>{auditTypeOptions.map((type) => <MenuItem key={type} value={type}>{type === 'all' ? 'All types' : type}</MenuItem>)}</TextField><Button variant="outlined" startIcon={<DownloadIcon />} disabled={!filteredRecords.length} onClick={handleDownload}>Download Excel</Button><Button variant="outlined" component="label" startIcon={<UploadFileIcon />} disabled={loading}>Upload Excel<input ref={uploadInput} hidden type="file" accept=".xlsx,.xls" onChange={handleUpload} /></Button><Button variant="contained" startIcon={<AddIcon />} onClick={handleAdd}>Add Completion</Button></Stack></Stack>
    {error && <Alert severity="error" sx={{ mb: 2.5 }} onClose={() => setError('')}>{error}</Alert>}
    <Grid container spacing={2.5} sx={{ mb: 3 }}><Grid size={{ xs: 12, lg: 7 }}><Card variant="outlined" sx={{ height: '100%' }}><CardContent><Stack direction="row" justifyContent="space-between" alignItems="flex-start"><Box><Typography variant="overline" color="text.secondary" fontWeight={800}>Monthly overview</Typography><Typography variant="h3" fontWeight={850} color="#0054A6">{filteredRecords.length}</Typography><Typography variant="body2" color="text.secondary">Completed audits in {month}{auditTypeFilter === 'all' ? '' : ` · ${auditTypeFilter}`}</Typography></Box><Chip label={`${totalUniqueItems.toLocaleString()} unique items`} color="primary" variant="outlined" /></Stack><Divider sx={{ my: 2 }} /><Table size="small"><TableHead><TableRow>{['S.No.', 'State', 'No. of Audits', 'Total No. of Unique Items'].map((header) => <TableCell key={header} sx={{ bgcolor: '#E7F0FB', color: '#0054A6', fontWeight: 800 }}>{header}</TableCell>)}</TableRow></TableHead><TableBody>{summary.length ? summary.map((item, index) => <TableRow key={item.state}><TableCell>{index + 1}</TableCell><TableCell>{item.state}</TableCell><TableCell>{item.auditCount}</TableCell><TableCell>{item.uniqueItems.toLocaleString()}</TableCell></TableRow>) : <TableRow><TableCell colSpan={4} align="center" sx={{ py: 3, color: 'text.secondary' }}>No records for this selection.</TableCell></TableRow>}<TableRow sx={{ bgcolor: '#DCEAF7' }}><TableCell /><TableCell sx={{ fontWeight: 800, color: '#004F98' }}>Total</TableCell><TableCell sx={{ fontWeight: 800, color: '#004F98' }}>{filteredRecords.length}</TableCell><TableCell sx={{ fontWeight: 800, color: '#004F98' }}>{totalUniqueItems.toLocaleString()}</TableCell></TableRow></TableBody></Table></CardContent></Card></Grid><Grid size={{ xs: 12, lg: 5 }}><Card variant="outlined" sx={{ height: '100%' }}><CardContent><Typography fontWeight={800}>Completed Audits by State</Typography><Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>Distribution for the selected month and audit type</Typography>{summary.length ? <Stack direction="row" spacing={3} alignItems="center"><Box sx={{ width: 150, height: 150, flexShrink: 0, borderRadius: '50%', background: pieBackground }} /><Stack spacing={1} sx={{ flex: 1 }}>{summary.map((item, index) => <Stack key={item.state} direction="row" justifyContent="space-between" spacing={1}><Stack direction="row" spacing={0.8} alignItems="center"><Box sx={{ width: 10, height: 10, borderRadius: '50%', bgcolor: chartColors[index % chartColors.length] }} /><Typography variant="body2" fontWeight={700}>{item.state}</Typography></Stack><Typography variant="body2" color="text.secondary">{item.auditCount} · {((item.auditCount / filteredRecords.length) * 100).toFixed(1)}%</Typography></Stack>)}</Stack></Stack> : <Typography variant="body2" color="text.secondary">Add or upload records to see the chart.</Typography>}</CardContent></Card></Grid></Grid>
    <Card variant="outlined"><CardContent sx={{ p: 0 }}>
      <Box sx={{ px: 2.5, py: 2 }}>
        <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'stretch', sm: 'center' }} spacing={1.5}>
          <Box>
            <Typography variant="h6" fontWeight={800}>Completion Records</Typography>
            <Typography variant="body2" color="text.secondary">
              Showing {tableRecords.length} of {filteredRecords.length} entries for {month}{auditTypeFilter === 'all' ? '' : ` · ${auditTypeFilter}`}.
              {hasActiveFilters && <Box component="span" sx={{ ml: 1 }}><Button size="small" variant="text" color="error" sx={{ p: 0, minWidth: 'auto', fontSize: 'inherit', textDecoration: 'underline' }} onClick={() => { setColumnFilters(emptyColumnFilters()); setPage(0); }}>Clear filters</Button></Box>}
            </Typography>
          </Box>
        </Stack>
      </Box>
      <Divider />
      <TableContainer sx={{ maxHeight: 600, overflowX: 'auto', scrollbarWidth: 'none', '&::-webkit-scrollbar': { display: 'none' } }}>
        <Table stickyHeader size="small" sx={{ minWidth: widths.reduce((sum, width) => sum + width, 0), tableLayout: 'fixed' }}>
          <TableHead>
            <TableRow>
              {headers.map((header, index) => (
                <TableCell key={header} align={header === 'Actions' ? 'center' : 'left'} sx={{ width: widths[index], minWidth: widths[index], bgcolor: '#E7F0FB', color: '#0054A6', fontWeight: 800, whiteSpace: 'nowrap', py: 1.5 }}>
                  {header}
                </TableCell>
              ))}
            </TableRow>
            <TableRow>
              {keys.map((key, index) => (
                <TableCell key={key} sx={{ width: widths[index], minWidth: widths[index], bgcolor: '#F4F8FD', py: 0.75, px: 0.75, top: 41 }}>
                  <TextField
                    size="small"
                    placeholder={`Filter…`}
                    ...
                    sx={{
                      ...
                      '& .MuiOutlinedInput-root': {
                        bgcolor: columnFilters[key] ? '#EBF3FF' : 'white',
                        '& fieldset': { borderColor: columnFilters[key] ? '#004F98' : '#D0D7E3' },
                      },
                    }}
                  />
                </TableCell>
              ))}
              <TableCell sx={{ width: widths[9], minWidth: widths[9], bgcolor: '#F4F8FD', top: 41 }} />
            </TableRow>
          </TableHead>
          <TableBody>
            {tableRecords.length ? paginatedRecords.map((record, index) => (
              <TableRow key={record._id || index} hover>
                {keys.map((key, keyIndex) => (
                  <TableCell key={key} sx={{ width: widths[keyIndex], minWidth: widths[keyIndex], whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} title={record[key]}>
                    {record[key]}
                  </TableCell>
                ))}
                <TableCell align="center" sx={{ width: widths[9], minWidth: widths[9] }}>
                  <Tooltip title="Edit completion"><IconButton size="small" color="primary" onClick={() => handleEdit(record)}><EditOutlinedIcon fontSize="small" /></IconButton></Tooltip>
                  <Tooltip title="Delete completion"><IconButton size="small" color="error" onClick={() => setRecordToDelete(record)}><DeleteOutlineIcon fontSize="small" /></IconButton></Tooltip>
                </TableCell>
              </TableRow>
            )) : (
              <TableRow>
                <TableCell colSpan={headers.length} align="center" sx={{ py: 7, color: 'text.secondary' }}>
                  {loading ? 'Loading completion records…' : hasActiveFilters ? 'No records match the active filters.' : 'No completion records found.'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
      ...
      <TablePagination component="div" count={tableRecords.length} page={page} onPageChange={(_, nextPage) => setPage(nextPage)} rowsPerPage={rowsPerPage} onRowsPerPageChange={(event) => { setRowsPerPage(Number(event.target.value)); setPage(0); }} rowsPerPageOptions={[10, 25, 50]} />
    </CardContent></Card>
