...
const AuditCompletion = () => {
  ...
  const tableRecords = useMemo(() => {
    ...
    return search ? filteredRecords.filter((record) => keys.some((key) => record[key].toLowerCase().includes(search))) : filteredRecords;
  ...
  return <Container maxWidth="xl" sx={{ py: { xs: 3, md: 4 }, pb: 8 }}>
    ...
    <Card variant="outlined"><CardContent sx={{ p: 0 }}><Box sx={{ px: 2.5, py: 2 }}><Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'stretch', sm: 'center' }} spacing={1.5}><Box><Typography variant="h6" fontWeight={800}>Completion Records</Typography><Typography variant="body2" color="text.secondary">Showing entries for {month}{auditTypeFilter === 'all' ? '' : ` · ${auditTypeFilter}`}.</Typography></Box><TextField size="small" label="Search records" placeholder="Site, dealer, state..." value={tableSearch} onChange={(event) => { setTableSearch(event.target.value); setPage(0); }} sx={{ minWidth: { sm: 250 } }} /></Stack></Box><Divider /><TableContainer sx={{ maxHeight: 560, overflowX: 'auto', scrollbarWidth: 'none', '&::-webkit-scrollbar': { display: 'none' } }}><Table stickyHeader size="small" sx={{ minWidth: widths.reduce((sum, width) => sum + width, 0), tableLayout: 'fixed' }}><TableHead><TableRow>{headers.map((header, index) => <TableCell key={header} align={header === 'Actions' ? 'center' : 'left'} sx={{ width: widths[index], minWidth: widths[index], bgcolor: '#E7F0FB', color: '#0054A6', fontWeight: 800, whiteSpace: 'nowrap', py: 1.5 }}>{header}</TableCell>)}</TableRow></TableHead><TableBody>{tableRecords.length ? paginatedRecords.map((record, index) => <TableRow key={record._id || index} hover>{keys.map((key, keyIndex) => <TableCell key={key} sx={{ width: widths[keyIndex], minWidth: widths[keyIndex], whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} title={record[key]}>{record[key]}</TableCell>)}<TableCell align="center" sx={{ width: widths[9], minWidth: widths[9] }}><Tooltip title="Edit completion"><IconButton size="small" color="primary" onClick={() => handleEdit(record)}><EditOutlinedIcon fontSize="small" /></IconButton></Tooltip><Tooltip title="Delete completion"><IconButton size="small" color="error" onClick={() => setRecordToDelete(record)}><DeleteOutlineIcon fontSize="small" /></IconButton></Tooltip></TableCell></TableRow>) : <TableRow><TableCell colSpan={headers.length} align="center" sx={{ py: 7, color: 'text.secondary' }}>{loading ? 'Loading completion records...' : 'No completion records match this search.'}</TableCell></TableRow>}</TableBody></Table></TableContainer><TablePagination component="div" count={tableRecords.length} page={page} onPageChange={(_, nextPage) => setPage(nextPage)} rowsPerPage={rowsPerPage} onRowsPerPageChange={(event) => { setRowsPerPage(Number(event.target.value)); setPage(0); }} rowsPerPageOptions={[10, 25, 50]} /></CardContent></Card>
